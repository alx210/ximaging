	...
	const char *msg_title, const char *msg_str)
{
	...
	wbox = XmCreateMessageDialog(wparent, dlg_type_name, args, i);
	...
	if(blocking){
		...
		add_delete_window_handler(XtParent(wbox),
			msgbox_delete_cb, (XtPointer)&result);
	} else {
		XtAddCallback(wbox, XmNokCallback, msgbox_cb, (XtPointer)&result);
	}
 ...
 */
char* rename_file_dlg(Widget wparent, char *file_title)
{
	static Widget wdlg;
	Arg arg[10];
	...
	wdlg = XmCreatePromptDialog(wparent, "renameDialog", arg, i);
	
	XtAddCallback(wdlg, XmNokCallback, input_dlg_cb, (XtPointer)&ret_string);
	...
	add_delete_window_handler(XtParent(wdlg), input_dlg_delete_cb, &ret_string);
 ...
 * Returns a valid string or NULL if cancelled.
 */
char* pass_to_input_dlg(Widget wparent)
{
	static Widget wdlg;
	Arg arg[8];
	Widget wtext;
	...
	wdlg = XmCreatePromptDialog(wparent,"passToDialog", arg ,i);
	XtAddCallback(wdlg, XmNokCallback, input_dlg_cb, (XtPointer)&ret_string);
	XtAddCallback(wdlg, XmNcancelCallback,
		input_dlg_cb, (XtPointer)&ret_string);
	XtUnmanageChild(XmSelectionBoxGetChild(wdlg, XmDIALOG_HELP_BUTTON));
...
/*
 * Displays a blocking select pattern input dialog.
 * Returns a valid string or NULL if cancelled.
 */
char* select_pattern_input_dlg(Widget wparent)
{
	static Widget wdlg;
	Arg arg[8];
	Widget wtext;
	...
	XtSetArg(arg[i], XmNtitle,
		nlstr(DLG_MSGSET, SID_SELPATTERN, "Select Pattern")); i++;
	XtSetArg(arg[i], XmNdialogStyle, XmDIALOG_PRIMARY_APPLICATION_MODAL); i++;
	XtSetArg(arg[i], XmNautoUnmanage, False); i++;

	wdlg = XmCreatePromptDialog(wparent, "selectPatternDialog",  arg , i);
	...
	const char *init_path)
{
	...
	wdlg = XmCreateFileSelectionDialog(wparent,"dirSelect",arg,i);
	...
	XtAddCallback(wdlg, XmNokCallback, input_dlg_cb, (XtPointer)&dir_name);
	XtAddCallback(wdlg, XmNcancelCallback, input_dlg_cb, (XtPointer)&dir_name);
	XtUnmanageChild(XmFileSelectionBoxGetChild(wdlg, XmDIALOG_HELP_BUTTON));
	
	add_delete_window_handler(XtParent(wdlg), input_dlg_delete_cb, &dir_name);
